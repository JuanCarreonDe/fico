-- ============================================================================
-- 0001_demo_billing_cutoff_day.sql  (SOLO FICO-demo mmesbruuevcosuuttblc)
-- Fecha de corte por tarjeta de crédito en la BD DEMO.
--
-- OJO: el schema de la demo es distinto al de FICO real:
--   * accounts NO tiene is_deleted (usa initial_balance y NO bytes is_deleted)
--     -> aquí usamos initial_balance y filtrOU is_archived=false (la demo no
--     tiene is_deleted en accounts; se confirma al regenerar types).
--   * El RETURNS TABLE de get_account_balances cambia (se añaden billing_cutoff_day,
--     credit_balance, credit_pending) => DROP + CREATE (42P13).
--
